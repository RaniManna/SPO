...
enum literal_type {
    LT_BOOL, LT_STR, LT_CHAR, LT_BITS, LT_DEC, LT_HEX
};
...
struct cfg_node {
    ...
    struct ast_node *body;
    struct cfg_node *connection_array[100];
    Agnode_t *agn_node;
    bool is_visited;

    bool is_visited_asm;
    struct cfg_node **parents;
};
...
// struct ast_node *set_type_operand(struct ast_node *, enum type_ref);

struct ast_node *set_type_array(struct ast_node *, struct ast_node *);

// struct ast_node *make_bool(const char *);
//
// struct ast_node *make_str(const char *);
//
// struct ast_node *make_chr(const char *);
//
// struct ast_node *make_bits(const char *);
//
// struct ast_node *make_dec(const char *);

// struct ast_node *set_ident_type(struct ast_node *, const char *);

struct ast_node *make_break();

struct ast_node *make_literal(const char *, struct ast_node *); // todo
...
struct ast_node *make_branch(struct ast_node *, struct ast_node *,
                             struct ast_node *);
...
// struct ast_node *set_array_type(struct ast_node *, const char *);

// void stmts_list_insert(struct ast_node **, struct ast_node *);

struct ast_node *make_program(struct ast_node *);

struct ast_node *insert_def_list(struct ast_node *, struct ast_node *);
